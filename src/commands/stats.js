...
export async function run(message) {
    const upS = Math.ceil(CLIENT.uptime % 60).toString().padStart(2, '0');
    const upM = Math.floor((CLIENT.uptime % (60 * 60)) / 60).toString().padStart(2, '0');
    const upH = Math.floor((CLIENT.uptime % (60 * 60 * 24)) / (60 * 60)).toString().padStart(2, '0');
    const upD = Math.floor(CLIENT.uptime / (60 * 60 * 24));
